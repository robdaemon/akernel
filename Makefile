...
.PHONY: all kernel userspace init serial fuzz spin echo initrd run clean clean-kernel clean-userspace clean-initrd
...
userspace: init serial fuzz spin echo
...
$(INITRD_IMG): init serial fuzz spin echo tools/mkinitrd.py
	rm -rf $(INITRD_ROOT)
	...
	printf '%s\n' 'program 2 Tests/Fuzz ipc_test echo_image' >> $(INITRD_ROOT)/System/Manifest
