...
.PHONY: all kernel userspace init serial initrd run clean clean-kernel clean-userspace clean-initrd
...
$(INITRD_IMG): init serial tools/mkinitrd.py
	rm -rf $(INITRD_ROOT)
	mkdir -p $(INITRD_ROOT)/System $(INITRD_ROOT)/Drivers $(INITRD_OUT)
