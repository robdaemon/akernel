...
.PHONY: all kernel userspace init serial fuzz spin memstage echo fileserver fat32 partmgr virtio_rng virtio_blk initrd run clean clean-kernel clean-userspace clean-initrd
...
$(INITRD_IMG): init serial fuzz spin memstage echo fileserver fat32 partmgr virtio_rng virtio_blk tools/mkinitrd.py
	rm -rf $(INITRD_ROOT)
