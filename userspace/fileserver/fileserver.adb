...
--  and the Receive side of the fs endpoint (handle 1). Init pushes
--  the (handle -> name) table as Op_Set_Name messages right after
--  spawn; until the table is complete (init sends one message per
--  boot file) Stat/Open/Read answer Status_Not_Ready.
--
...
procedure Fileserver is
   ...
   function Find (Name : String; Len : Natural) return Natural is
   begin
      for I in File_Table'Range loop
         if File_Table (I).Valid
           and then File_Table (I).Name_Len = Len
           and then File_Table (I).Name (1 .. Len) = Name (1 .. Len)
         then
            return I;
         end if;
      end loop;
...
begin
   ...
   loop
      ...
      if Syscalls.Message.Label = Files.Op_Set_Name then
