...
procedure Echo_Server is
   ...
   package RPC is new Aegir_User.IPC (Echo_Request, Echo_Response);
   ...
   Rounds            : Natural := 0;
begin
   Aegir_User.Console.Set_Endpoint (2);
   Aegir_User.Console.Put_Line ("echo online");
   ...
   if Status = IPC_Ok then
      Aegir_User.Console.Put_Line ("echo done");
   else
      Aegir_User.Console.Put_Line ("echo error exit");
   end if;
