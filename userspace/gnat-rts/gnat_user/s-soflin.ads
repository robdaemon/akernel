...
package System.Soft_Links is
   ...
   --  53c addition: the expander (exp_disp) references this type
   --  when building dispatch tables for controlled types with
   --  iterator interfaces (Ada.Containers.Vectors, used by the
   --  vendored Ada.Directories). The embedded pool's spec dropped
   --  it; copied verbatim from the full s-soflin.ads. Never
   --  referenced at run time in this non-tasking runtime.
