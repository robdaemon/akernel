/*  Aegir runtime glue (milestone 53a; slimmed in 53b): the full
 ...
 *  lands in newlib's malloc, backed by Aegir_User.Gloss's _sbrk
...
extern void aegir_sys_exit (unsigned long long code) __attribute__((noreturn));

void
__gnat_exit (int status)
{
   aegir_sys_exit ((unsigned long long) status);
   for (;;) {}
}
