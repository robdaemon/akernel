/*  Aegir GNAT.Sockets C port layer (milestone 73): the ABI
 *  reference is gcc-15.3.0 gcc/ada/socket.c (the file the M72 plan
 *  called "gsocket.c") plus the imports of g-socthi/g-sothco.  All
 *  socket operations ride Aegir_User.Sockets (the aknet_sock_*
 *  exports of userspace/rts/aegir/aegir_user-sockets.adb), which
 ...
 *  aegir socket ABI, sized by gnat_user/s-oscons.ads.
 *
...
/* ------------------------------------------------------------------ */
/*  Aegir bridge (Aegir_User.Sockets C exports)                    */
/* ------------------------------------------------------------------ */
...
/*  Raw syscall stub (userspace/rts/aegir/syscalls-riscv64.s) */
extern ak_u64 aegir_sys_sleep_until (ak_u64 deadline);

/*  Tick clock: the riscv `time` CSR at 10 MHz, read from U-mode
 *  (aegir_rtclock.c) — the domain Sleep_Until deadlines live in.
 *  NOTE: syscall 34 (read_clock) is the WALL clock (seconds/ns
 *  from the RTC), a different domain — do not use it here. */
extern ak_u64 aegir_rdtime (void);
...
/*  Bridge protocol statuses (aegir_user-sockets.ads) */
#define AK_ST_OK        0
...
struct ak_sock
{
   ...
   ak_u64         handle;       /*  aegir socket cap */
