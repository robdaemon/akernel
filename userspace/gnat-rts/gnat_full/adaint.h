...
#ifndef AKERNEL_NO_DIRENT
#include <dirent.h>
#else
/*  newlib for riscv64-elf has no <dirent.h> (no libgloss dir
    syscalls). Ada.Directories (53c) will route through our fs
    Op_ReadDir instead; until then the dir entry points in
    adaint.c compile to returning-failure stubs. */
typedef void DIR;
