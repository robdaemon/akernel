...
kernel introspection (37b: userspace Proc: fs driver over
process_info — VFS-mounted process tree; 37a landed the
syscall), register fast path,
...
Recently landed: MILESTONE 37a — process introspection
syscall (Sys_Process_Info = 30). Writes a 64-byte binary
snapshot (pid, SPAWNER pid recorded at spawn — new PCB
field, lifecycle, thread state, cap count, IPC flags,
blocked-on endpoint, call badge) into a caller memobj cap
(Write right) via physmap per-word. Authority reuses the
device_resource Kernel_Object+Manage gate; init grants it
down with the plain device_resource manifest token (fuzz:
handle 7). a1 = slot 0..31 or U64'Last self (works for
kernel-started init, which has no table slot); statuses
0/1-not-found/U64'Last. Enumeration idiom = 32 fixed
by-index queries over the sparse table (Op_ReadDir style).
Pids alias across slot reuse (pid = slot+4) — accepted.
Burn: initrd System/Manifest is GENERATED by the Makefile
— edit the recipe, not initrd/root. 227 PASS SMP1+SMP4,
fuzz failures=0, fsck clean. Next: 37b Proc: fs driver
(Op_Add_FS volume served by a System/Procfs process,
renders snapshots as Proc:<pid>/status + Proc:tree; no
Proc:self — VFS forwarding hides the caller's badge).
Before that: MILESTONE 36 — assigns (Amiga-style
